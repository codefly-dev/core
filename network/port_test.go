...
func TestPortHostModeMatchesLegacy(t *testing.T) {
	...
	native := network.ToNamedPort(ctx, "mind-server", "mind", "mind", "grpc", standards.GRPC, network.PortModeFor("native"))
	nix := network.ToNamedPort(ctx, "mind-server", "mind", "mind", "grpc", standards.GRPC, network.PortModeFor("nix"))
...
func TestPortContainerModeDiffersFromHost(t *testing.T) {
	ctx := context.Background()
	host := network.ToNamedPort(ctx, "mind-server", "mind", "mind", "grpc", standards.GRPC, network.PortModeHost)
	container := network.ToNamedPort(ctx, "mind-server", "mind", "mind", "grpc", standards.GRPC, network.PortModeContainer)
