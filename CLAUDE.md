...
- **companions/** — Sidecar containers for language tooling:
  ...
  - Core ships the Dockerfile and the build spec (`companions.BuildSpecs()`); the CLI builds and publishes. Build locally with `codefly companion build --all` (or `codefly companion build <name>`); publishing is `codefly companion publish --all`, run from the CLI repository's Companions workflow.
...
# Companion Docker images (local build, in dependency order; the CLI publishes)
