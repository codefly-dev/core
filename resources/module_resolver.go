	...
	if workspace.overlay != nil {
		if directive := workspace.overlay.Resolve[ref.Name]; directive != nil {
			...
			case directive.Worktree != "":
				...
				checkout, err := findWorktreeCheckout(ctx, workspace.Dir(), repo, gitRef)
				if err != nil {
...
// findWorktreeCheckout scans the worktree roots for a checkout of repo with ref
// checked out, and returns its checkout-root directory. Identity is confirmed by
// the checkout's own origin remote (authoritative — the directory-name
// convention only says where to look). An exact branch-name match is preferred;
// otherwise any checkout whose HEAD resolves to ref is used. Ambiguity (more
// than one match at the same precedence) is a hard error rather than a silent
// pick.
func findWorktreeCheckout(ctx context.Context, fromDir, repo, ref string) (string, error) {
	container := worktreeContainer(fromDir)
	if container == "" {
		return "", fmt.Errorf("cannot locate worktree container from %q (expected a github-<org>-<repo>/<branch> layout)", fromDir)
	}
	...
	if err != nil {
		return "", fmt.Errorf("cannot read worktree container %q: %w", container, err)
	}
	for _, orgDir := range orgDirs {
		...
		for _, branchDir := range branchDirs {
			...
			candidateRepo, ok := gitOriginRepo(ctx, root)
			if !ok || candidateRepo != wantRepo {
				continue
			}
			if gitBranch(ctx, root) == ref {
				exact = append(exact, root)
			} else if gitRefAtHead(ctx, root, ref) {
				fallback = append(fallback, root)
			}
		}
	}
	...
	return "", fmt.Errorf("no local worktree of %s has %s checked out under %s", repo, ref, container)
}
