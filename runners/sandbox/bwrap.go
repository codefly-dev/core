...
// runtime failure.
//
// **NetworkLoopback on Linux.** bwrap's --unshare-net creates an isolated
// network namespace and configures its loopback device before dropping the
// setup capabilities. The payload must run directly: attempting to configure
// loopback again after that drop fails with EPERM on unprivileged hosts.
...
// ErrNetworkLoopbackUnsupported is retained for backend parity.
...
func (s *bwrapSandbox) buildArgs() []string {
	...
	// --unshare-net creates an isolated namespace whose only configured
