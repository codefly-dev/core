...
Run buf checks through the `Makefile`, never a bare `buf`: its targets pin the
version CI's gate and the companion's generator both use. Generation runs buf
inside the published companion, not from the host's `PATH`.
...
- **Regenerate protos with `codefly generate proto --proto ./proto --output
  ./generated --local`.** `--local` selects `generated/buf.gen.local.yaml`;
  buf, plugins and goimports run inside the pinned proto companion. Do not
  hand-run `buf`/`protoc`. Commit the `.proto` change and the
  regenerated bindings together; see [`proto/README.md`](proto/README.md).
