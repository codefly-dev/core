...
- **Regenerate protos with `codefly generate proto --proto ./proto --output
  ...
  It does not pin buf — it execs whatever is on `PATH` — so run `make
  buf-install` first, or the bindings you commit were generated by a buf nothing
  here agrees with (codefly-dev/cli#744, which pins it in the tool where it
  belongs). Do not hand-run `buf`/`protoc`. Commit the `.proto` change and the
