...
- **Regenerate protos with `codefly generate proto --proto ./proto --output
  ./generated --local`.** `--local` pins plugin versions and runs goimports so
  the output is byte-reproducible. Do not hand-run `buf`/`protoc`. Commit the
  `.proto` change and the regenerated bindings together; see
