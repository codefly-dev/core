...
func generatedGoFiles(root string) ([]string, error) {
	...
	err := filepath.WalkDir(root, func(file string, entry os.DirEntry, err error) error {
		...
		if !entry.Type().IsRegular() {
			return fmt.Errorf("generated Go candidate %s is not a regular file", file)
		}
		parsed, err := parser.ParseFile(token.NewFileSet(), file, nil, parser.PackageClauseOnly|parser.ParseComments)
		if err != nil {
			return err
		}
