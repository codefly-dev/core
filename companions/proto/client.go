...
func generateClient(ctx context.Context, spec clientSpec) error {
	...
	// protoc-gen-es names a file it was not asked to generate by package only
	// for the well-known types, which it publishes as @bufbuild/protobuf/wkt.
	// Every other import — google/api, buf/validate — it names by a path
	// relative to the file it emits, and buf generates nothing for a dependency
	// module, so the bindings would import files nothing wrote and tsc fails
	// with TS2307 on each one. The TypeScript template asks buf for those files;
	// this decides when, and the destination's reclaim reads the same answer.
	//
	...
	if includeImports {
		...
		// not a set known ahead of time, so the destination is reclaimed by what
		// this run owns rather than by a list of what it does not.
		if err = removeUnownedOutput(ctx, spec.destination, spec.sources); err != nil {
			return w.Wrapf(err, "cannot remove stale bindings")
		}
	} else if err = removeForeignOutput(ctx, spec.destination); err != nil {
...
//
// A TypeScript run from sources reclaims through removeUnownedOutput instead:
// it generates the imports, so the set it writes is not these roots.
...
// removeUnownedOutput deletes every directory of destination this run does not
// generate into.
//
// removeForeignOutput can name what to reclaim because the set is fixed: two
// namespace roots the library never owns. Asking buf for the imports removes
// that guarantee — the run writes a tree for every namespace the sources
// import, so a contract that drops an import leaves the tree behind, which is
// the same drift removeForeignOutput exists to prevent and the reason a
// library keeps shipping bindings generated by a protoc-gen-es it no longer
// runs. What stays fixed is the other side: a run owns the directories its own
// sources generate into, and everything else under destination came from a
// previous run.
//
// An ancestor of an owned directory is kept — reclaiming buf/validate from a
// module whose own protos live under buf/ would delete the run's own output.
func removeUnownedOutput(ctx context.Context, destination string, sources []Source) error {
	w := wool.Get(ctx).In("removeUnownedOutput", wool.DirField(destination))
	owned := map[string]bool{".": true}
	for _, src := range sources {
		for dir := path.Dir(filepath.ToSlash(src.Path)); dir != "." && dir != "/"; dir = path.Dir(dir) {
			owned[dir] = true
		}
	}
	entries, err := os.ReadDir(destination)
	if err != nil {
		return w.Wrapf(err, "cannot read destination")
	}
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		if err = removeUnownedTree(ctx, destination, entry.Name(), owned); err != nil {
			return err
		}
	}
	return nil
...
func removeUnownedTree(ctx context.Context, destination, rel string, owned map[string]bool) error {
	w := wool.Get(ctx).In("removeUnownedTree", wool.DirField(destination))
	full := filepath.Join(destination, filepath.FromSlash(rel))
	if !owned[rel] {
		if err := os.RemoveAll(full); err != nil {
			return w.Wrapf(err, "cannot remove %s", full)
		}
		return nil
	}
	entries, err := os.ReadDir(full)
	if err != nil {
		return w.Wrapf(err, "cannot read %s", full)
	}
	for _, entry := range entries {
		if !entry.IsDir() {
			continue
		}
		if err = removeUnownedTree(ctx, destination, rel+"/"+entry.Name(), owned); err != nil {
			return err
		}
	}
	return nil
