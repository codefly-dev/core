...
// through docs/runbooks/bump-protobuf-es.md: the pins asserted below are three
// of the six places this version lives, and the other three — the ts facade
// lockfile and the flake's two content hashes — have no static guard at all.
...
func TestBumpRunbookNamesTheCurrentPin(t *testing.T) {
	const runbook = "../../docs/runbooks/bump-protobuf-es.md"
	content, err := os.ReadFile(runbook)
	if err != nil {
	...
	if match == nil {
		t.Fatalf("%s has no `## Current decision: stay on <version>` heading; that heading is what this guard reads", runbook)
	}
	if got := string(match[1]); got != protocGenEsRuntimeVersion {
		t.Errorf("%s records the decision as %s, but the companion pins %s; the runbook documents a pin that moved", runbook, got, protocGenEsRuntimeVersion)
	}
}
