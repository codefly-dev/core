...
func TestGeneratedGoFilesRejectSymlinks(t *testing.T) {
	root := t.TempDir()
	outside := filepath.Join(t.TempDir(), "user.go")
	contents := []byte("// Code generated by protoc-gen-go. DO NOT EDIT.\npackage user\n")
	require.NoError(t, os.WriteFile(outside, contents, 0o600))
	require.NoError(t, os.Symlink(outside, filepath.Join(root, "linked.go")))
	_, err := generatedGoFiles(root)
