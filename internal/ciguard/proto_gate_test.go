...
// CI's buf and the buf the proto companion bakes must be the same one. The
// companion generates every consumer's bindings; CI decides whether a schema
// change is allowed to land. Run them on different versions and the gate can
// pass a schema the generator then chokes on.
//
...
// companion's 1.71.0 for exactly that reason.
